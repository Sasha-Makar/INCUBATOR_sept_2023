...
div.innerText = 'Hello my name is Johny';
...
coursesBlock.classList.add('courses');
document.body.appendChild(coursesBlock);

for (const block of coursesAndDurationArray) {
    let div = document.createElement('div');
    let title = document.createElement('h2');
    title.innerText = block.title + ' - ' + block.monthDuration;
    div.appendChild(title);

    coursesBlock.appendChild(div);
}
...
for (let course of coursesAndDurationArray) {
    ...
    h1.innerText = `Course Title -  ${course.title}`;
    let p = document.createElement('p');
    p.classList.add('description');
    p.innerText = `Course Duration - ${course.monthDuration}`;
...
for (let item of simpsons) {
    ...
    divInfo.innerText = ` Info- ${item.info}`;
    let img = document.createElement('img');
    img.src = item.photo;

    divMember.append(name, surname, age, divInfo, img);
...
for (const item of coursesArray) {
    ...
    title.innerText = `Title of course - ${item.title}`;
    let duration = document.createElement('p');
    duration.innerText = `Duration - ${item.monthDuration} monthes`;
    let hour = document.createElement('p');
    hour.innerText = ` Number of hours - ${item.hourDuration} houres`;
    ...
    divCourse.append(title, duration,hour, ul);
