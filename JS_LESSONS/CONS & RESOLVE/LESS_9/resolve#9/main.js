...
for (const item of coursesAndDurationArray) {
    // console.log(item);
    let div = document.createElement('div');
    div.innerText = `${item.title} - duration ${item.monthDuration}`;
...
for (const item of simpsons) {
    ...
    h2.innerText = `${item.name} ${item.surname}, age - ${item.age}`;
    let p = document.createElement('p');
...
for (const course of coursesArray) {
    // console.log(course);
        ...
        h3.innerText = `monthes - ${course.monthDuration}, hours - ${course.hourDuration}`;
        let ul = document.createElement('ul');
